...
After acceptance the form records a sent receipt and clears its busy state before opening the thank-you page. A boolean receipt marker in that history entry restores confirmation even when Back reloads the page instead of using its cache. No form answers are stored in history. The receipt includes a deliberate link to start another inquiry; analytics or navigation failures cannot enable duplicate retries.

Phone visitors can jump directly to the form. Name fields use a full row on phones and tablets. The website comparison keeps its plan headings and feature labels in view within a bounded scroll region; the three-column Ads comparison fits narrow screens.
...
For browser form and performance checks, build with `PUBLIC_FORM_DELIVERY=server` and run `node tools/qa-server.mjs`. The printed loopback URL serves the built site with local performance observations and a mocked form provider. `QA Failure` in the name field simulates rejection; other valid inquiries simulate acceptance. `/__qa/status` reports counts without inquiry details. No inquiry is transmitted externally. Measurements are from this local fixture on the test computer, not PageSpeed scores or real-user field data. The published server never loads these diagnostics.

The fixture's `qa_mode` query parameter supports `reduced-motion`, `save-data`, `webgl-unavailable`, `no-js`, `lifecycle`, `context-loss`, and `large-type`. Preference/lifecycle modes simulate the signals consumed by the app without changing browser settings; `no-js` blocks scripts with a response policy, and `context-loss` exercises the actual WebGL context-loss extension. `large-type` sets the root font size to 200% as a layout stress check; it does not simulate full browser zoom. Diagnostic callbacks measure CPU work, not GPU frame time. Set `QA_PORT` only when a stable local port is needed.
