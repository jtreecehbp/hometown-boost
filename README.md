...
- The ascent passes two striped hot-air balloons, an orange-and-white propeller plane, and a gold satellite with blue solar panels. `src/scripts/launch-landmarks.ts` builds them as five batched meshes at separate world altitudes. The plane crosses with scroll; gentle bobbing, propeller motion, and satellite rotation share the existing pause controls. Portrait layouts use smaller models and closer lanes.
